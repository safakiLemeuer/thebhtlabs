...
/* ═══════════════ SVG ICON LIBRARY — see const I below ═══════════════ */

/* ═══════════════ LOCALE SYSTEM — USD default, EUR for Europe ═══════════════ */
const LOCALES = {
  us: {
    ...
    industries: [
      {v:"financial",l:"Financial Services & Banking",avg:56},{v:"healthcare",l:"Healthcare & Life Sciences",avg:49},
      {v:"technology",l:"Technology / SaaS",avg:64},{v:"professional",l:"Professional Services",avg:55},
      {v:"manufacturing",l:"Manufacturing",avg:44},{v:"govdef",l:"Government / Defense",avg:48},
      {v:"retail",l:"Retail / E-Commerce",avg:52},{v:"energy",l:"Energy / Utilities",avg:47},
      {v:"education",l:"Education",avg:51},{v:"insurance",l:"Insurance",avg:53},
      {v:"legal",l:"Legal",avg:41},{v:"construction",l:"Construction / Real Estate",avg:40},{v:"other",l:"Other",avg:49},
    ],
    ...
    painPoints: [
      "Manual processes consuming too many hours","Data scattered across multiple systems","Compliance gaps (CMMC, HIPAA, SOX, etc.)",
      "Employees resistant to new technology","AI tools deployed but not delivering ROI","No clear AI strategy or roadmap",
      "Difficulty measuring operational efficiency","Cybersecurity or data privacy concerns",
      "Client/regulator asking about AI governance","Competitor pressure to adopt AI faster",
    ],
  },
  eu: {
    ...
    regIndustryMap: {
      financial: ["EU AI Act","GDPR","DORA","PSD2","MiFID II"], healthcare: ["EU AI Act","GDPR","MDR","IVDR","NIS2"],
      govdef: ["EU AI Act","GDPR","NIS2","eIDAS 2.0"], insurance: ["EU AI Act","GDPR","DORA","Solvency II"],
      legal: ["EU AI Act","GDPR","NIS2"], energy: ["EU AI Act","GDPR","NIS2","REMIT"],
      technology: ["EU AI Act","GDPR","NIS2","Data Act","Digital Services Act"],
    },
    certBadges: "Microsoft Partner · ISO 27001 · ISO 42001 Aligned · GDPR Compliant",
    phone: "", email: "eu@bhtsolutions.com",
    industries: [
      {v:"financial",l:"Financial Services & Banking",avg:60},{v:"healthcare",l:"Healthcare & Life Sciences",avg:52},
      {v:"technology",l:"Technology & SaaS",avg:65},{v:"professional",l:"Professional Services",avg:57},
      {v:"manufacturing",l:"Manufacturing & Industrial",avg:48},{v:"govdef",l:"Government & Public Sector",avg:50},
      {v:"retail",l:"Retail & E-Commerce",avg:53},{v:"energy",l:"Energy & Utilities",avg:51},
      {v:"education",l:"Education & Research",avg:49},{v:"insurance",l:"Insurance",avg:55},
      {v:"legal",l:"Legal Services",avg:43},{v:"construction",l:"Construction & Real Estate",avg:42},{v:"other",l:"Other",avg:50},
    ],
    empRanges: ["1-10","11-50","51-200","201-500","501-1,000","1,001-5,000","5,001-10,000","10,000+"],
    revRanges: ["< €1M","€1M - €5M","€5M - €25M","€25M - €100M","€100M - €500M","€500M+","Prefer not to say"],
    painPoints: [
      "EU AI Act compliance uncertainty","GDPR + AI integration challenges","Data scattered across multiple systems",
      "No AI risk classification framework","AI tools deployed but not delivering ROI","No clear AI strategy or roadmap",
...
/* Detect locale: EU countries get EUR, everyone else USD */
const EU_CODES = ['DE','AT','CH','FR','BE','LU','NL','IT','ES','PT','IE','FI','SE','DK','NO','PL','CZ','GR','RO','HU','BG','HR','SK','SI','LT','LV','EE','CY','MT','LI','IS'];
const getLocale = () => {
  if (typeof window === 'undefined') return 'us';
  const p = new URLSearchParams(window.location.search).get('locale');
  if (p && LOCALES[p]) return p;
...
/* ═══════════════ ARIA Score™ — Engagement Complexity Calculator ═══════════════ */
const calcARIA = (intake, dScores, locale) => {
  const loc = LOCALES[locale] || LOCALES.us;
  // 6 dimensions scored 1-5
  const empN = parseInt((intake.employees||"").replace(/[^0-9]/g,'')||"10");
  const sizeScore = empN>=5000?5:empN>=1000?4:empN>=200?3:empN>=50?2:1;

  const revStr = intake.revenue||"";
  const revScore = /500|2B|\+/.test(revStr)?5:/100/.test(revStr)?4:/25/.test(revStr)?3:/5M|5 Cr/.test(revStr)?2:1;

  const regCount = (loc.regIndustryMap?.[intake.industry]||loc.regulatory||[]).length;
  const regScore = regCount>=6?5:regCount>=4?4:regCount>=3?3:regCount>=2?2:1;

  const techScore = Math.ceil((dScores.tech||50)/20);
  const dataScore = (dScores.governance||50)>=80?5:(dScores.governance||50)>=60?4:(dScores.governance||50)>=40?3:2;
  const scopeScore = (dScores.usecase||50)>=80?5:(dScores.usecase||50)>=60?4:(dScores.usecase||50)>=40?3:2;

  const dimensions = [
    {name:"Organization Size",score:sizeScore,max:5},
    {name:"Revenue Scale",score:revScore,max:5},
    {name:"Regulatory Exposure",score:regScore,max:5},
    {name:"Technical Footprint",score:techScore,max:5},
    {name:"Data Sensitivity",score:dataScore,max:5},
    {name:"AI Adoption Scope",score:scopeScore,max:5},
  ];
  const total = dimensions.reduce((a,d)=>a+d.score,0); // 6-30

  const tier = total>=24?{label:"Enterprise",color:"#DC2626",mult:2.5}
    :total>=18?{label:"Mid-Market",color:"#EA580C",mult:1.8}
    :total>=12?{label:"Growth",color:"#2563EB",mult:1.2}
    :{label:"Foundational",color:"#0E7490",mult:1.0};
  ...
  const combinedMult = Math.round(tier.mult * sector.mult * 10) / 10;
  const base = loc.pricing;
  const fmt = loc.currencyFmt;

  return {
    total, tierLabel:tier.label, tierColor:tier.color, mult:tier.mult,
    sectorLabel:sector.label, sectorMult:sector.mult, sectorDesc:sector.desc, combinedMult,
    dimensions, fmt,
    pricing: {
      scan:{adjusted:Math.round(base.scan*combinedMult/100)*100,hours:Math.round(10*Math.max(1,combinedMult*.8))},
      sprint:{adjusted:Math.round(base.sprint*combinedMult/100)*100,hours:Math.round(30*Math.max(1,combinedMult*.8))},
      launchpad:{adjusted:Math.round(base.launchpad*combinedMult/100)*100,hours:Math.round(40*Math.max(1,combinedMult*.8))},
    },
  };
};
...
/* ═══════════════ ICON SYSTEM — Professional SVG icons (no emoji) ═══════════════ */
const I = {
  // Assessment domain icons (24x24)
  database: (c="#0D9488",s=20)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4.03 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4.03 3 9 3s9-1.34 9-3V5"/></svg>,
  workflow: (c="#0D9488",s=20)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/><path d="M10 8.5h4.5a2 2 0 012 2V14"/><path d="M14 8.5l-2 -2m2 2l-2 2"/></svg>,
  server: (c="#0D9488",s=20)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="8" rx="2"/><rect x="2" y="14" width="20" height="8" rx="2"/><circle cx="6" cy="6" r="1" fill={c}/><circle cx="6" cy="18" r="1" fill={c}/></svg>,
  users: (c="#0D9488",s=20)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>,
  target: (c="#0D9488",s=20)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>,
  shield: (c="#0D9488",s=20)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M9 12l2 2 4-4"/></svg>,
  compass: (c="#0D9488",s=20)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76" fill={c} opacity=".15" stroke={c}/></svg>,
  // Federal Hub icons (24x24)
  building: (c="#F59E0B",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 21h18"/><path d="M5 21V7l7-4 7 4v14"/><path d="M9 21v-4h6v4"/><path d="M9 10h.01M15 10h.01M9 14h.01M15 14h.01"/></svg>,
  bot: (c="#F59E0B",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="8" width="18" height="12" rx="2"/><path d="M12 8V5"/><circle cx="12" cy="3" r="2"/><circle cx="9" cy="14" r="1.5" fill={c}/><circle cx="15" cy="14" r="1.5" fill={c}/></svg>,
  clipboard: (c="#F59E0B",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M16 4h2a2 2 0 012 2v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/><path d="M9 14l2 2 4-4"/></svg>,
  zap: (c="#F59E0B",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>,
  barChart: (c="#5EEAD4",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="12" width="4" height="9" rx="1"/><rect x="10" y="7" width="4" height="14" rx="1"/><rect x="17" y="3" width="4" height="18" rx="1"/></svg>,
  shieldLock: (c="#5EEAD4",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><rect x="9" y="11" width="6" height="5" rx="1"/><path d="M10 11V9a2 2 0 014 0v2"/></svg>,
  alertTriangle: (c="#5EEAD4",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><circle cx="12" cy="16" r=".5" fill={c}/></svg>,
  fileText: (c="#5EEAD4",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="13" x2="15" y2="13"/><line x1="9" y1="17" x2="15" y2="17"/></svg>,
  crosshair: (c="#5EEAD4",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="22" y1="12" x2="18" y2="12"/><line x1="6" y1="12" x2="2" y2="12"/><line x1="12" y1="6" x2="12" y2="2"/><line x1="12" y1="22" x2="12" y2="18"/></svg>,
  lock: (c="#5EEAD4",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/><circle cx="12" cy="16" r="1" fill={c}/></svg>,
  cloud: (c="#5EEAD4",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M18 10h-1.26A8 8 0 109 20h9a5 5 0 000-10z"/></svg>,
  graduationCap: (c="#5EEAD4",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 10l-10-5L2 10l10 5 10-5z"/><path d="M6 12v5c0 1.66 2.69 3 6 3s6-1.34 6-3v-5"/><line x1="22" y1="10" x2="22" y2="16"/></svg>,
  trendingUp: (c="#5EEAD4",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>,
  handshake: (c="#5EEAD4",s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M20 17l-3.5-3.5"/><path d="M4 17l3.5-3.5"/><path d="M7.5 13.5L11 10l2 2 4-4"/><path d="M2 9l4.5-4.5a2 2 0 012.83 0L11 6"/><path d="M22 9l-4.5-4.5a2 2 0 00-2.83 0L13 6"/><path d="M2 9l2 8h3l2-3"/><path d="M22 9l-2 8h-3l-2-3"/></svg>,
  // Partner/Contact tab icons
  phone: (c=C.teal,s=16)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>,
  link: (c=C.teal,s=16)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>,
};
...
const AQ = [
  ...
  {d:"Technology Readiness",icon:"server",q:["Are you using cloud-based tools (M365, Google Workspace, AWS)?","Can your systems integrate with third-party APIs?","Do you have a cybersecurity baseline (MFA, endpoint protection)?","Are your systems on supported/current software versions?","Do employees have access to collaboration tools (Teams, Slack)?"]},
  {d:"People & Culture",icon:"graduationCap",q:["Is leadership open to experimenting with AI tools?","Do employees currently use any AI tools (ChatGPT, Copilot)?","Is there budget allocated for technology training?","Would your team embrace AI assistance or resist change?","Do you have someone who understands AI basics?"]},
  {d:"Strategy & ROI",icon:"crosshair",q:["Can you identify 3+ tasks consuming >5 hrs/week that are repetitive?","Do you have clear KPIs for operational efficiency?","Would saving 10-20 hrs/week per team member impact revenue?","Are competitors in your industry already adopting AI?","Do you have budget flexibility for a 3-6 month pilot?"]},
...
function Assessment({id}) {
  ...
  const activePains = loc.painPoints;
    ...
    <section id={id} style={{padding:"80px 0",background:C.bgSoft}}>
      <div style={{maxWidth:720,margin:"0 auto",padding:"0 24px",textAlign:"center"}}>
        ...
        <div style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:20,padding:"48px 40px",boxShadow:C.shadowMd}}>
          <div style={{marginBottom:16,display:"flex",justifyContent:"center"}}>{I.compass(C.teal,48)}</div>
    ...
    return (
      <section id={id} style={{padding:"80px 0",background:C.bgSoft}}>
        <div style={{maxWidth:800,margin:"0 auto",padding:"0 24px"}}>
          <SH tag={`Results for ${intake.company}`} title="Your AI Readiness Score" />
            ...
            <div style={{padding:28}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginTop:16}}>
                <button onClick={()=>{setPhase("landing");setStep(0);setAns({});setSaved(false);}}
                  style={{padding:"10px",borderRadius:10,border:`1px solid ${C.border}`,background:C.bg,cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:F.h,color:C.textMuted}}>"Retake Assessment"</button>
                <button onClick={generatePDF}
                  style={{padding:"10px",borderRadius:10,border:"none",background:C.teal,cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:F.h,color:"#fff"}}>"Download PDF Report"</button>
                <button onClick={bookDiscovery} disabled={discoveryStatus==="sent"||discoveryStatus==="sending"}
                  ...
                  {discoveryStatus===""?"Book Discovery Call":discoveryStatus==="sending"?"Sending...":discoveryStatus==="sent"?"Booked!":"Retry"}
                </button>
              </div>
    ...
    <section id={id} style={{padding:"80px 0",background:C.bgSoft}}>
      <div style={{maxWidth:1000,margin:"0 auto",padding:"0 24px"}}>
        ...
        <div style={{display:"flex",gap:4,marginBottom:24,background:C.bgMuted,padding:4,borderRadius:12,maxWidth:440}}>
          {[{k:"contact",l:<><span style={{display:"inline-flex",verticalAlign:"middle",marginRight:6}}>{I.phone()}</span>Get in Touch</>,d:"Hire us or ask a question"},{k:"partner",l:<><span style={{display:"inline-flex",verticalAlign:"middle",marginRight:6}}>{I.link()}</span>Apply to Partner</>,d:"Join our partner network"}].map(t=>(
        ...
        {tab==="contact" ? (
          <div className="g2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              {[
                {icon:I.crosshair(C.teal,18),t:"Hire Us",d:"AI, cloud, security expertise. 2-week sprints to 12-month engagements. Cleared resources.",c:C.teal},
                {icon:I.handshake(C.violet,18),t:"Partner",d:"Prime contractors: SBA 8(a), EDWOSB, WOSB certified. Cleared resources. Let's team.",c:C.violet},
                {icon:I.target(C.blue,18),t:"Assess",d:"35-point AI evaluation. Cloud readiness. Security posture. Executive briefing.",c:C.blue},
                {icon:I.graduationCap(C.coral,18),t:"Train",d:"Custom AI, cloud, security training for 5-200 people. In-person or virtual.",c:C.coral},
              ].map(c => (
          ...
          <div style={{maxWidth:700}}>
            <div style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:18,padding:32,boxShadow:C.shadowMd}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24}}>
                {[
                  {i:I.shield(C.teal,14),t:"Proprietary IP",d:"ARIA Score™ framework, diagnostic tools, assessment platform"},
                  {i:I.target(C.teal,14),t:"Lead Flow",d:"Assessment leads in your territory, pre-qualified and scored"},
                  {i:I.graduationCap(C.teal,14),t:"Training & Cert",d:"Quarterly training, methodology certification, quality standards"},
                  {i:I.compass(C.teal,14),t:"Brand Authority",d:"Co-branded materials, \"BHT Certified Partner\" badge, joint marketing"},
                ].map(c=>(
                  <div key={c.t} style={{padding:14,borderRadius:10,background:C.bgSoft,border:`1px solid ${C.borderLight}`}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                      <span style={{display:"flex"}}>{c.i}</span>
    ...
    <section id="fed-tools" style={{padding:"80px 0",background:"#0F172A"}}>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"0 24px"}}>
        ...
        <div className="g2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          {[
            {icon:"building",title:"M-25-21 Compliance Readiness Check",desc:"8-question self-assessment mapped to the 7 pillars of OMB M-25-21. Instantly see which requirements your agency has met and where gaps remain. Outputs a printable compliance gap summary your CAIO can action today.",
              tags:["Free","AI Governance","CAIO Ready"],action:"Take Assessment →",target:"assess"},
            {icon:"bot",title:"AI Bot Governance Auditor",desc:"Enter any .gov website. We scan for chatbot widgets and evaluate their AI governance posture against M-25-21 transparency requirements, NIST AI RMF controls, and accessibility standards. 10 seconds.",
              tags:["Free","No Credentials","Instant"],action:"Audit a Site →",target:"assess"},
            {icon:"clipboard",title:"AI Use Case Inventory Template",desc:"M-25-21 requires agencies to annually inventory and publicly publish AI use cases. Download our structured template with fields mapped to OMB's reporting requirements, including risk classification and high-impact AI designation.",
              tags:["Free","Template","M-25-21 §3(b)"],action:"Download Template",download:true},
            {icon:"zap",title:"AI Risk Classification Matrix",desc:"Determine which of your AI use cases qualify as 'high-impact' under M-25-21 Section 6. Interactive flowchart covering: healthcare, law enforcement, benefits, safety-critical, and autonomous systems. Get your risk tier in 2 minutes.",
            ...
            <div key={tool.title} style={{padding:28,borderRadius:16,background:"#1E293B",border:"1px solid #334155",transition:"all .2s",position:"relative",overflow:"hidden"}}>
              <div style={{marginBottom:12}}>{I[tool.icon]&&I[tool.icon]("#F59E0B",28)}</div>
    ...
    <section style={{padding:"80px 0",background:"#0F172A"}}>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"0 24px"}}>
        ...
        <div className="g3" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
          {[
            {icon:"barChart",title:"AI Use Case Inventory",price:"From $18K",desc:"Comprehensive catalog of AI deployments across your agency. Risk classification per M-25-21 §6. High-impact designation. Public inventory preparation.",mandate:"M-25-21 §3(b)(ii)"},
            {icon:"shieldLock",title:"AI Governance Framework",price:"From $35K",desc:"Establish or strengthen your AI Governance Board. CAIO support structure. Policies, procedures, and minimum risk management practices for high-impact AI.",mandate:"M-25-21 §3-4"},
            {icon:"alertTriangle",title:"High-Impact AI Compliance",price:"From $28K",desc:"Assess existing AI systems against M-25-21 minimum practices. Develop impact assessments, risk mitigation plans, and waiver documentation. April 2026 deadline-ready.",mandate:"M-25-21 §4, §6"},
            {icon:"fileText",title:"AI Acquisition Support",price:"From $15K",desc:"Develop AI-specific solicitation provisions per M-25-22. Contract clauses for data ownership, IP rights, vendor disclosure, American-made AI preferences.",mandate:"M-25-22 §3-5"},
            {icon:"crosshair",title:"AI Strategy Development",price:"From $45K",desc:"Agency-wide AI strategy aligned to mission priorities and OMB requirements. Innovation roadmap. Resource allocation. Cross-agency sharing opportunities.",mandate:"M-25-21 §2"},
            {icon:"lock",title:"CMMC + AI Governance",price:"From $55K",desc:"Unified cybersecurity and AI compliance. CMMC Level 2 preparation with AI governance overlay. 110/110 NIST 800-171 + AI risk management in a single engagement.",mandate:"CMMC 2.0 + M-25-21"},
            {icon:"cloud",title:"GCC-High AI Deployment",price:"From $65K",desc:"Deploy AI capabilities in GCC-High environments. Copilot Studio, Azure AI Foundry, and Teams AI bots in FedRAMP-authorized infrastructure.",mandate:"FedRAMP + M-25-22"},
            {icon:"graduationCap",title:"AI Workforce Upskilling",price:"From $12K",desc:"Train agency staff on prompt engineering, RAG architecture, responsible AI, and AI use case identification. Custom curriculum aligned to your agency's AI strategy.",mandate:"M-25-21 §2(c)"},
            {icon:"trendingUp",title:"AI Performance Monitoring",price:"From $22K/yr",desc:"Ongoing monitoring and evaluation of AI system performance, risks, and effectiveness. Quarterly assessments. Annual inventory updates. Continuous M-25-21 compliance.",mandate:"M-25-21 §4 + M-25-22"},
          ].map(svc=>(
            <div key={svc.title} style={{padding:22,borderRadius:14,background:"#1E293B",border:"1px solid #334155",display:"flex",flexDirection:"column"}}>
              <span style={{marginBottom:8,display:"flex"}}>{I[svc.icon]&&I[svc.icon]("#5EEAD4",22)}</span>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"0 24px"}}>
        ...
        <div style={{padding:24,borderRadius:16,background:"#0F172A",border:"1px solid #334155"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
            <span style={{display:"flex"}}>{I.handshake("#5EEAD4",24)}</span>
            <div>
...
function TenantHealthCheck({id}) {
  ...
  const si = (s) => s==='pass'?{icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.teal} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M9 12l2 2 4-4"/></svg>,color:C.teal,bg:'#F0FDFA'}:s==='warn'?{icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#EA580C" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><circle cx="12" cy="16" r=".5" fill="#EA580C"/></svg>,color:'#EA580C',bg:'#FFF7ED'}:{icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.rose} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>,color:C.rose,bg:'#FEF2F2'};
